...
const pool = new Pool({
  connectionString:
    ...
    "postgresql://postgres:postgres@localhost:5432/dj_grey_db",
...
function authenticateAdmin(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    if (err)
      return res.status(403).json({ error: "Invalid or expired login token." });
    if (user.role !== "admin")
...
function authenticateUser(req, res, next) {
  ...
  if (!token)
    return res
      .status(401)
      .json({ error: "Please log in to perform this action." });
...
function authenticateDJOrAdmin(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    ...
    if (user.role !== "admin" && user.role !== "dj")
      return res.status(403).json({ error: "DJ privileges required." });
    req.user = user;
    next();
  });
}
...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Registration failed. Email may already exist or API error.",
      });
  }
});
...
app.get("/api/auth/verify/:token", async (req, res) => {
  try {
    ...
    await pool.query("UPDATE users SET status = 'approved' WHERE email = $1", [
      decoded.email,
    ]);
    res.send(
...
app.post("/api/mixes/:id/like", authenticateUser, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      newLikes: updatedMix.rows[0].likes_count,
      liked,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post("/api/mixes/:mixId/comments", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ ...result.rows[0], username: req.user.username, likes_count: 0 });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post("/api/admin/livestream", authenticateDJOrAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: is_active ? "Livestream launched!" : "Livestream ended.",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/mixes", authenticateAdmin, async (req, res) => {
  ...
  artwork_url = artwork_url
    ? artwork_url.replace("dl=0", "raw=1")
...
app.delete("/api/mixes/:id", authenticateAdmin, async (req, res) => {
  try {
    await pool.query("DELETE FROM mixes WHERE id = $1", [req.params.id]);
    res.json({ success: true });
  } catch (err) {}
});
...
app.post(
  ...
  async (req, res) => {
    try {
      const sub = (
        await pool.query("SELECT * FROM mix_submissions WHERE id = $1", [
          req.params.id,
        ])
      ).rows[0];
      await pool.query(
      ...
      await pool.query(
        "INSERT INTO mixes (title, audio_url, artwork_url, likes_count, downloads_count, user_id, is_active) VALUES ($1, $2, $3, 0, 0, $4, true)",
        [
          `${sub.title} (by DJ ${sub.dj_name})`,
...
app.get("/api/spotify/sync", async (req, res) => {
  try {
    ...
    let dbCheck = await pool.query("SELECT * FROM mixes WHERE audio_url = $1", [
      spotifyUrl,
    ]);
    let spotifyMix =
    ...
    res.status(500).json({ error: "Spotify sync error" });
  }
});
